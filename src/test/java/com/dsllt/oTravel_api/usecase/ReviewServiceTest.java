...
//import com.dsllt.oTravel_api.infra.adapter.review.in.web.model.CreateReviewDTO;
...
//    @BeforeEach
//    public void setup(){
//        this.reviewService = new ReviewService(reviewJpaRepository, placeJpaRepository, userJpaRepository);
//    }
//
//    @Test
...
//                .build();
//        // Act
//        when(reviewJpaRepository.save(any(Review.class))).thenReturn(repositorySavedReview);
//        when(placeJpaRepository.findById(placeId)).thenReturn(Optional.ofNullable(newPlace));
//        when(userJpaRepository.findById(userId)).thenReturn(Optional.ofNullable(newUser));
//
//        Review savedReview = reviewService.save(createReviewDTO);
//
//        // Assert
//        then(reviewJpaRepository).should().save(reviewArgumentCaptor.capture());
...
//        verify(reviewJpaRepository, times(1)).save(reviewArgumentCaptor.capture());
//    }
//
//    @Test
...
//        when(reviewJpaRepository.findAll()).thenReturn(reviewList);
//
//        // Act
...
//                .createdAt(LocalDateTime.now())
//                .build();
//        when(reviewJpaRepository.findById(reviewUuid)).thenReturn(Optional.of(savedReview));
//
//        // Act
...
//        // Arrange
//        UUID reviewUuid = UUID.randomUUID();
//        when(reviewJpaRepository.findById(reviewUuid)).thenThrow(new ObjectNotFoundException("Review não encontrada."));
//
//        // Act
...
//                .createdAt(LocalDateTime.now())
//                .build();
//        when(reviewJpaRepository.findById(reviewUuid)).thenReturn(Optional.of(retrievedReview));
//        when(placeJpaRepository.findById(placeId)).thenReturn(Optional.of(newPlace));
//        when(userJpaRepository.findById(userId)).thenReturn(Optional.of(newUser));
//        when(reviewJpaRepository.save(any(Review.class))).thenReturn(updatedReview);
//
//        // Act
